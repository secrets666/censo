...
class cmip_dataset(Dataset):
    def __init__(self, sst_cmip6, nino_cmip6, samples_gap):
        ...
        if len(sst1) >= 1001:
            predData = np.load(r'/media/ruichuang/ENSO_predict/predictedENSO/interval_5_7/CMIP5_pred_5.npy')
            print("predData.shape=", predData.shape)
        elif len(sst1) <= 1000:
            predData = np.load(r'/media/ruichuang/ENSO_predict/predictedENSO/interval_5_7/GODAS_pred.npy')
            print("predData.shape=", predData.shape)
