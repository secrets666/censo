...
class SAConvLSTM(nn.Module):
    ...
    def forward(self, input_x, input_y=None, device=torch.device('cuda:0'), input_frames=12, future_frames=26, output_frames=37,
        ...
        """
        '''input_x=(N,38,24,48,6) input_y=(N,38,3)'''
        assert len(input_x.shape) == 5
        input_x = input_x.to(device)
        input_y = input_y.to(device)
        pred_x = input_x[:, :, :, :, :2]  # (N,38,24,48,2)
        input_x = input_x[:, :, :, :, 2:]  # (N,38,24,48,4)
        ...
        pred_x = torch.cat((pred_x[:,1:,:,:,:], pred_x[:,:1,:,:,:]), dim=1)
        input_x = torch.cat((input_x, pred_x), dim=-1)
        input_x = input_x.permute(0,1,4,2,3)  # (N,38,6,24,48)
...
if __name__ == "__main__":
    ...
    input_x = torch.rand((2, 38, 24, 48, 6))
