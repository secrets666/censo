...
class Trainer:
    ...
    def train_once(self, sst, nino_true, ratio):
        ...
        # print(sst.shape, nino_true.shape)  # (2,38,24,48,6) (2,38,3)
        ...
        loss_sst = self.loss_sst(sst_pred, sst[:, -num_t:, :, :, 2:4].permute(0, 1, 4, 2, 3).to(self.device))
        loss_nino = self.loss_nino(nino_pred, nino_true[:,12:36,0].float().to(self.device))
        loss = loss_sst + loss_nino
        loss.backward()
    ...
    def test(self, dataloader_test):
        ...
        with torch.no_grad():
            for sst, nino_true in dataloader_test:
                sst, nino = self.network(sst.float(), nino_true, device=self.device, train=False)
    ...
    def infer(self, dataset, dataloader, save_data=False):
        ...
        with torch.no_grad():
            ...
            sst_true = torch.from_numpy(dataset.sst[:, 12:, :, :, 2:4]).permute(0, 1, 4, 2, 3).float().to(self.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_dim_1', '-hd1', default=(32, 32, 32, 32), type=lambda x: tuple(map(int, x.split(','))), help='hiddem dim')
    ...
    parser.add_argument('--d_attn_1', '-da1', default=16, type=int, help='d attn')
    ...
    parser.add_argument('--epoch', '-e', default=50, type=int, help='')
    ...
    dataset_train = cmip_dataset(sst_train, nino_train, samples_gap=7)
    ...
    print('testing...')
    loss_sst_test, loss_nino_test, sc_test, cor = trainer.infer(dataset=dataset_test, dataloader=dataloader_test, save_data=False)
    print(cor)
