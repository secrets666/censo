...
class Trainer:
    ...
    def infer(self, dataset, dataloader, save_data=False):
        ...
        with torch.no_grad():
            ...
            sst_true = torch.from_numpy(dataset.sst[:, :, :, :, 2:4]).permute(0, 1, 4, 2, 3).float().to(self.device)
            ...
            # not train data path, retrain data path
            if save_data:
                sst_pred = sst_pred.to("cpu").numpy()
                print("sst_pred.shape=", sst_pred.shape)
                np.save('/media/ruichuang/ENSO_predict/predictedENSO/interval_7_11/GODAS_pred.npy', sst_pred)
                sst_true = sst_true.to("cpu").numpy()
                print("sst_true.shape=", sst_true.shape)
                np.save('/media/ruichuang/ENSO_predict/predictedENSO/interval_7_11/GODAS_true.npy', sst_true)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--d_attn_1', '-da1', default=32, type=int, help='d attn')
    ...
    print('testing...')
    loss_sst_test, loss_nino_test, sc_test, cor = trainer.infer(dataset=dataset_test, dataloader=dataloader_test, save_data=True)
    print(cor)
