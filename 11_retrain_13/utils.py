...
class cmip_dataset(Dataset):
    def __init__(self, sst_cmip6, nino_cmip6, samples_gap):
        ...
        # train data path, not retrain data path
        if len(sst1) >= 1001:
            predData = np.load(r'/media/ruichuang/ENSO_predict/predictedENSO/interval_7_11/CMIP5_pred_13.npy')
            print("predData.shape=", predData.shape)
        elif len(sst1) <= 1000:
            predData = np.load(r'/media/ruichuang/ENSO_predict/predictedENSO/interval_7_11/GODAS_pred.npy')
            print("predData.shape=", predData.shape)
