...
def prepare_inputs_targets(len_time, input_gap, input_length, pred_shift, pred_length, samples_gap, start):
...
class cmip_dataset(Dataset):
    def __init__(self, sst_cmip6, nino_cmip6, samples_gap, start=None):
        super().__init__()
        ...
        if sst_cmip6 is not None:
            ...
            idx_sst = prepare_inputs_targets(sst_cmip6.shape[0], input_gap=1, input_length=12,
                                             pred_shift=26, pred_length=26, samples_gap=samples_gap, start=start)
        ...
        if len(sst1) >= 1001:
            predData = np.load(r'/media/ruichuang/ENSO_predict/predictedENSO/interval_5/CMIP5_pred_3.npy')
